...
namespace org.applied_geodesy.adjustment
{
	...
	{
		private readonly Matrix covarianceMatrix;
		private readonly int dimension;
		private double[] eigenvalues; private double[] minimalDetectableBias;
...
//ORIGINAL LINE: public ConfidenceRegion(org.applied_geodesy.adjustment.statistic.TestStatisticParameters testStatisticParameters, no.uib.cipr.matrix.Matrix covarianceMatrix) throws IllegalArgumentException, no.uib.cipr.matrix.NotConvergedException
		public ConfidenceRegion(TestStatisticParameters testStatisticParameters, Matrix covarianceMatrix) : this(covarianceMatrix)
		{
			this.testStatisticParameters = testStatisticParameters;
		}

//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
//ORIGINAL LINE: public ConfidenceRegion(no.uib.cipr.matrix.Matrix covarianceMatrix) throws IllegalArgumentException, no.uib.cipr.matrix.NotConvergedException
		public ConfidenceRegion(Matrix covarianceMatrix)
		{
			if (!covarianceMatrix.isSquare())
			{
				throw new System.ArgumentException(this.GetType() + " Matrix must be a squared matrix!");
			}
			...
			for (int i = 0; i < covarianceMatrix.numColumns(); i++)
			{
				...
				{
					if (double.IsInfinity(covarianceMatrix.get(i,j)) || double.IsNaN(covarianceMatrix.get(i,j)))
					{
						throw new System.ArgumentException(this.GetType() + " Matrix contains Inf or NaN values! " + i + "x" + j + " " + covarianceMatrix.get(i,j));
					}
				}
			}
		...
		/// <summary>
		/// Liefert die Halbachse der Ellipse/des Ellispoids </summary>
		/// <param name="index"> </param>
		/// <returns> axis </returns>
		public virtual double getConfidenceAxis(int index)
		{
		...
		/// <summary>
		/// Liefert den Eigenwert, Eigenwerte sind der Groesse nach geordnet </summary>
		/// <param name="index"> </param>
		/// <returns> eigenvalue </returns>
		public virtual double getEigenvalue(int index)
		{
			return this.eigenvalues[this.sortOrder[index]];
		}

		/// <summary>
		/// Liefert die Eigenvektoren, sind in Abhaengigkeit der Eigenwerte der Groesse nach geordnet </summary>
		/// <param name="index"> </param>
		/// <returns> eigenvector </returns>
		public virtual Vector getEigenVector(int index)
		{
		...
		/// <param name="index"> </param>
		/// <returns> m </returns>
		public virtual double getMinimalDetectableBias(int index)
		{
			return this.minimalDetectableBias[index];
		}

//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
//ORIGINAL LINE: private void SVD() throws no.uib.cipr.matrix.NotConvergedException
		private void SVD()
		{
	...
	//		this.minimalDetectableBias = Matrices.getArray(maxEvalVecRot);
		...
		/// <summary>
		/// Ermittelt die Sortierreihenfolge der Eigenwerte und speichert die Indizes in einem Array </summary>
		...
		/// <summary>
		/// Liefert die Halbachse der 2D-Konfidenzellipse </summary>
		/// <param name="index"> </param>
		/// <param name="isHelmertEllipse"> </param>
		/// <returns> axis </returns>
		public virtual double getConfidenceAxis2D(int index, bool isHelmertEllipse)
		{
		...
		/// <summary>
		/// Liefert dem Drehwinkel der 2D-Konfidenzellipse </summary>
		...
		/// <summary>
		/// Liefert das Quantil in Abhaengigkeit der Dimension </summary>
		...
		private double getQuantile(int dimension)
		{
			...
			{
				return this.testStatisticParameters.getTestStatisticParameter(dimension, double.PositiveInfinity).Quantile;
			}
			return 1.0;
		}
	}
