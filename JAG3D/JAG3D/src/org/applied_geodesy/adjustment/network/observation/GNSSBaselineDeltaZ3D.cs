...
namespace org.applied_geodesy.adjustment.network.observation
{
	...
	public class GNSSBaselineDeltaZ3D : GNSSBaseline3D
	{
	...
	//	fz = m*(  Math.sin(ry)*dX - Math.sin(rx)*Math.cos(ry)*dY + Math.cos(rx)*Math.cos(ry)*dZ)
		...
		public GNSSBaselineDeltaZ3D(int id, Point startPoint, Point endPoint, double observation, double sigma) : base(id, startPoint, endPoint, observation, sigma)
		{
		}
