...
//ORIGINAL LINE: public void setInitialGuess(double nx, double ny, double nz, double d) throws IllegalArgumentException
		public virtual void setInitialGuess(double nx, double ny, double nz, double d)
		{
		...
		public override Point CenterOfMass
		{
			set
			{
				...
				double dist = d.Value;
				dist += (nx * prevCenterOfMass.X0 + ny * prevCenterOfMass.Y0 + nz * prevCenterOfMass.Z0);
				dist -= (nx * currCenterOfMass.X0 + ny * currCenterOfMass.Y0 + nz * currCenterOfMass.Z0);
				d.Value = dist;
			}
		}
		...
		public override void reverseCenterOfMass(UpperSymmPackMatrix Dp)
		{
			...
			if (Dp != null)
			{
				...
				if (d.Column >= 0)
				{
					...
					if (nz.Column >= 0)
					{
						J.set(d.Column, nz.Column, centerOfMass.Z0);
					}
			...
			double dist = d.Value + nx.Value * centerOfMass.X0 + ny.Value * centerOfMass.Y0 + nz.Value * centerOfMass.Z0;
			d.Value = dist;
		}
