...
//ORIGINAL LINE: public void setInitialGuess(double x0, double y0, double z0, double a, double c, double r11, double r12, double r13, double r21, double r22, double r23, double r31, double r32, double r33) throws IllegalArgumentException
		public virtual void setInitialGuess(double x0, double y0, double z0, double a, double c, double r11, double r12, double r13, double r21, double r22, double r23, double r31, double r32, double r33)
		{
		...
		public override void jacobianElements(FeaturePoint point, Matrix Jx, Matrix Jv, int rowIndex)
		{
			...
			if (Jx != null)
			{
				if (X0.Column >= 0)
				{
					Jx.set(rowIndex, X0.Column, -2.0 * (r11 * ui * a * a + r21 * vi * c * c - r31 * wi));
				}
				if (Y0.Column >= 0)
				{
					Jx.set(rowIndex, Y0.Column, -2.0 * (r12 * ui * a * a + r22 * vi * c * c - r32 * wi));
				}
				if (Z0.Column >= 0)
				{
					Jx.set(rowIndex, Z0.Column, -2.0 * (r13 * ui * a * a + r23 * vi * c * c - r33 * wi));
				}
				...
				if (C.Column >= 0)
				{
					Jx.set(rowIndex, C.Column, 2.0 * c * vi * vi);
				}
				...
				if (R21.Column >= 0)
				{
					Jx.set(rowIndex, R21.Column, 2.0 * c * c * vi * (xi - x0));
				}
				if (R22.Column >= 0)
				{
					Jx.set(rowIndex, R22.Column, 2.0 * c * c * vi * (yi - y0));
				}
				if (R23.Column >= 0)
				{
					Jx.set(rowIndex, R23.Column, 2.0 * c * c * vi * (zi - z0));
				}

				if (R31.Column >= 0)
				{
					Jx.set(rowIndex, R31.Column, -2.0 * wi * (xi - x0));
				}
				if (R32.Column >= 0)
				{
					Jx.set(rowIndex, R32.Column, -2.0 * wi * (yi - y0));
				}
				if (R33.Column >= 0)
				{
					Jx.set(rowIndex, R33.Column, -2.0 * wi * (zi - z0));
				}
			}
		...
		{
			return "$a^2 u_i^2 + c^2 v_i^2 = w_i^2" + " \\\\ " + "\\left(\\begin{array}{c} u_i \\\\ v_i \\\\ w_i \\end{array}\\right) = \\mathbf{R} \\left(\\begin{array}{c} x_i-x_0 \\\\ y_i-y_0 \\\\ z_i-z_0 \\end{array}\\right);" + " \\\\ " + "\\mathbf{R} = \\left(\\begin{array}{ccc} r_{11} & r_{12} & r_{13} \\\\ r_{21} & r_{22} & r_{23} \\\\ r_{31} & r_{32} & r_{33} \\end{array}\\right)$";
		}
	}
}