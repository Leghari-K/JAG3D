...
namespace org.applied_geodesy.juniform.ui.dialog
{
	...
	public class AverageRestrictionDialog
	{
		...
		{
			private readonly AverageRestrictionDialog outerInstance;

			public RegressorEventHandler(AverageRestrictionDialog outerInstance)
			{
			...
			{
				if (@event.getSource() == outerInstance.addParameterButtom)
				{
					UnknownParameter parameter = outerInstance.regressorComboBox.getValue();
					if (parameter != null)
					{
						outerInstance.regressorListView.getItems().add(parameter);
					}
				}
				else if (@event.getSource() == outerInstance.removeParameterButtom)
				{
					UnknownParameter parameter = outerInstance.regressorListView.getSelectionModel().getSelectedItem();
					if (parameter != null)
					{
						outerInstance.regressorListView.getItems().remove(parameter);
					}
				}
			}
		}
		...
		public static Optional<AverageRestriction> showAndWait(ObservableList<UnknownParameter> unknownParameters, AverageRestriction averageRestriction)
		{
			...
			restrictionTypeDialog.Restriction = averageRestriction;
		...
		private AverageRestriction Restriction
		{
			set
			{
				...
				this.regressorListView.setItems(this.restriction.Regressors);
				this.latexLabel.Tex = this.restriction.toLaTex();
    
				this.regressorListView.setDisable(value.Indispensable);
				this.regressorComboBox.setDisable(value.Indispensable);
				this.regressandComboBox.setDisable(value.Indispensable);
    
				this.addParameterButtom.setDisable(value.Indispensable);
				this.removeParameterButtom.setDisable(value.Indispensable);
		...
		private class CallbackAnonymousInnerClass : Callback<ButtonType, AverageRestriction>
		{
			private readonly AverageRestrictionDialog outerInstance;

			public CallbackAnonymousInnerClass(AverageRestrictionDialog outerInstance)
			{
				this.outerInstance = outerInstance;
			}

			public override AverageRestriction call(ButtonType buttonType)
			{
		...
		private Node createPane()
		{
			...
			buttonBoxA.getChildren().addAll(this.addParameterButtom, this.removeParameterButtom);
