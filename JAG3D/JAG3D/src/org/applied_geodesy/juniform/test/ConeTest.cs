...
namespace org.applied_geodesy.juniform.test
{
	...
	public class ConeTest : NISTTest
	{
		private ConeTest()
		{
		}
	...
	//		1 number represents the full apex angle of the cone in degrees (less than 180)
			...
			double[] references = new double[] {x0Ref, y0Ref, z0Ref, xApexRef, yApexRef, zApexRef, nxRef, nyRef, nzRef, 1.0 / Math.Tan(phiRef), 1.0 / Math.Tan(phiRef)};
			...
			foreach (UnknownParameter unknownParameter in unknownParameters)
			{
				...
				{
					a = unknownParameter.Value;
				}
				else if (unknownParameter.ParameterType == ParameterType.MINOR_AXIS_COEFFICIENT)
				{
					c = unknownParameter.Value;
				}
			}
			...
			double[] solution = new double[] {x0, y0, z0, xApex, yApex, zApex, nx, ny, nz, a, c};

			IList<ParameterType> types = new List<ParameterType> {ParameterType.COORDINATE_X, ParameterType.COORDINATE_Y, ParameterType.COORDINATE_Z, ParameterType.ORIGIN_COORDINATE_X, ParameterType.ORIGIN_COORDINATE_Y, ParameterType.ORIGIN_COORDINATE_Z, ParameterType.ROTATION_COMPONENT_R31, ParameterType.ROTATION_COMPONENT_R32, ParameterType.ROTATION_COMPONENT_R33, ParameterType.MAJOR_AXIS_COEFFICIENT, ParameterType.MINOR_AXIS_COEFFICIENT};
