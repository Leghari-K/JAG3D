...
namespace org.applied_geodesy.jag3d.ui.graphic.layer
{
	...
	public class PointResidualArrowLayer : ArrowLayer
	{
		private IList<PointLayer> referenceLayers = FXCollections.observableArrayList();

		internal PointResidualArrowLayer(LayerType layerType) : base(layerType)
		{
			...
			case org.applied_geodesy.jag3d.ui.graphic.layer.LayerType.InnerEnum.POINT_RESIDUAL_HORIZONTAL:
				...
				try
				{
					symbolSize = double.Parse(PROPERTIES.getProperty("POINT_RESIDUAL_HORIZONTAL_ARROW_SYMBOL_SIZE"));
				}
				catch (Exception)
				{
				}
				try
				{
					lineWidth = double.Parse(PROPERTIES.getProperty("POINT_RESIDUAL_HORIZONTAL_ARROW_LINE_WIDTH"));
				}
				catch (Exception)
				{
				}
			...
			case org.applied_geodesy.jag3d.ui.graphic.layer.LayerType.InnerEnum.POINT_RESIDUAL_VERTICAL:
				...
				try
				{
					symbolSize = double.Parse(PROPERTIES.getProperty("POINT_RESIDUAL_VERTICAL_ARROW_SYMBOL_SIZE"));
				}
				catch (Exception)
				{
				}
				try
				{
					lineWidth = double.Parse(PROPERTIES.getProperty("POINT_RESIDUAL_VERTICAL_ARROW_LINE_WIDTH"));
				}
				catch (Exception)
				{
				}
			...
			foreach (PointLayer layer in this.referenceLayers)
			{
				if (layer.Visible)
				{
					...
					foreach (GraphicPoint startPoint in layer.Points)
					{
						...
						{
						case org.applied_geodesy.jag3d.ui.graphic.layer.LayerType.InnerEnum.POINT_RESIDUAL_HORIZONTAL:
							endPoint = new WorldCoordinate(startPoint.Coordinate.X + startPoint.ResidualX, startPoint.Coordinate.Y + startPoint.ResidualY);
							break;

						case org.applied_geodesy.jag3d.ui.graphic.layer.LayerType.InnerEnum.POINT_RESIDUAL_VERTICAL:
							endPoint = new WorldCoordinate(startPoint.Coordinate.X + 0, startPoint.Coordinate.Y + startPoint.ResidualZ);
							break;
						default:
							continue;
						}
			...
			foreach (PointLayer layer in this.referenceLayers)
			{
				if (layer.Visible && layer.hasContent())
				{
					foreach (GraphicPoint startPoint in layer.Points)
					{
						if (startPoint.Visible)
						{
							if ((layerType == LayerType.POINT_RESIDUAL_HORIZONTAL && startPoint.Dimension != 1) || (layerType == LayerType.POINT_RESIDUAL_VERTICAL && startPoint.Dimension != 2))
							{
								return true;
							}
						}
