...
namespace org.applied_geodesy.jag3d.ui.dialog
{
	...
	public class ImportOptionDialog
	{
		...
		public static Window Owner
		{
			set
			{
				importOptionDialog.window = value;
			}
		}
		...
		private class CallbackAnonymousInnerClass : Callback<ButtonType, ImportOption>
		{
			private readonly ImportOptionDialog outerInstance;

			public CallbackAnonymousInnerClass(ImportOptionDialog outerInstance)
			{
				this.outerInstance = outerInstance;
			}

			public override ImportOption call(ButtonType buttonType)
			{
	...
	//		gridPane.setMinWidth(250);
		...
		private void load()
		{
			...
			try
			{
				SQLManager.Instance.loadImportPreferences();
			...
			catch (Exception e)
			{
				...
				Platform.runLater(() =>
				{
				OptionDialog.showThrowableDialog(i18n.getString("ImportOptionDialog.message.error.load.exception.title", "Unexpected SQL-Error"), i18n.getString("ImportOptionDialog.message.error.load.exception.header", "Error, could not load import preferences from database."), i18n.getString("ImportOptionDialog.message.error.load.exception.message", "An exception has occurred during database transaction."), e);
				});
			}
		}
		...
		private void save()
		{
			try
			{
				...
				SQLManager.Instance.saveImportPreferences();
			}
			catch (Exception e)
			{
				Console.WriteLine(e.ToString());
				...
				Platform.runLater(() =>
				{
				OptionDialog.showThrowableDialog(i18n.getString("ImportOptionDialog.message.error.save.exception.title", "Unexpected SQL-Error"), i18n.getString("ImportOptionDialog.message.error.save.exception.header", "Error, could not save import preferences to database."), i18n.getString("ImportOptionDialog.message.error.save.exception.message", "An exception has occurred during database transaction."), e);
				});
			}
			finally
			{
