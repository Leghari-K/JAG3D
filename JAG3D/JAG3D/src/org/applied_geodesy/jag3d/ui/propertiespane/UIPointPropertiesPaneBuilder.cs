...
namespace org.applied_geodesy.jag3d.ui.propertiespane
{
	...
	public class UIPointPropertiesPaneBuilder
	{
		...
		private IDictionary<TreeItemType, UIPointPropertiesPane> propertiesPanes = FXCollections.observableHashMap();

		private UIPointPropertiesPaneBuilder()
		{
		}

		public static UIPointPropertiesPaneBuilder Instance
		{
			get
			{
		...
		public virtual UIPointPropertiesPane getPointPropertiesPane(TreeItemType type)
		{
			if (!this.propertiesPanes.ContainsKey(type))
			{
				this.propertiesPanes[type] = new UIPointPropertiesPane(type);
			}
			UIPointPropertiesPane propertiesPane = this.propertiesPanes[type];
