...
namespace org.applied_geodesy.jag3d.ui.propertiespane
{
	...
	public class UIObservationPropertiesPaneBuilder
	{
		...
		private IDictionary<TreeItemType, UIObservationPropertiesPane> propertiesPanes = FXCollections.observableHashMap();

		private UIObservationPropertiesPaneBuilder()
		{
		}

		public static UIObservationPropertiesPaneBuilder Instance
		{
			get
			{
		...
		public virtual UIObservationPropertiesPane getObservationPropertiesPane(TreeItemType type)
		{
			if (!this.propertiesPanes.ContainsKey(type))
			{
				this.propertiesPanes[type] = new UIObservationPropertiesPane(type);
			}
			UIObservationPropertiesPane propertiesPane = this.propertiesPanes[type];
